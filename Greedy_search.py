...
class search_maze:
    def __init__(self,name):
        ...
        for i in range(len(self.maze)):
            for j in range(len(self.maze[0])):
                if self.maze[i][j]=='P':
                    self.current=(10000,i,j,None)     #heuristic,row,column,parent
                if self.maze[i][j]=='.':
                    self.dest=(0,i,j,None)   
        self.visitmap=[[0 for x in range(len(self.maze[0]))] for y in range(len(self.maze))]
        self.visitmap[self.current[1]][self.current[2]]=1
        self.path_cost = 0
        self.node_number = 0
        self.frontier = queue.PriorityQueue()    #use priority queue as frontier,h as comparing factor 
    ...
    def Greedy(self):
        ...
        if (self.maze[self.current[1]][self.current[2]-1]!='%') & (self.visitmap[self.current[1]][self.current[2]-1]==0): #chek if able to go left
            #first calucluate heuristic, push new position into frontier,mark as visited
            h=abs(self.dest[1]-self.current[1])+abs(self.dest[2]-(self.current[2]-1))
            childleft=(h,self.current[1],self.current[2]-1,self.current)
        ...
        if (self.maze[self.current[1]][self.current[2]+1]!='%') & (self.visitmap[self.current[1]][self.current[2]+1]==0):    #go right
            h=abs(self.dest[1]-self.current[1])+abs(self.dest[2]-(self.current[2]+1))
            childright=(h,self.current[1],self.current[2]+1,self.current)
        ...
        if (self.maze[self.current[1]+1][self.current[2]]!='%') & (self.visitmap[self.current[1]+1][self.current[2]]==0):   #go down
            h=abs(self.dest[1]-(self.current[1]+1))+abs(self.dest[2]-self.current[2])
            childdown=(h,self.current[1]+1,self.current[2],self.current)
        ...
        if (self.maze[self.current[1]-1][self.current[2]]!='%') & (self.visitmap[self.current[1]-1][self.current[2]]==0==0):    #go up
            h=abs(self.dest[1]-(self.current[1]-1))+abs(self.dest[2]-self.current[2])
            childup=(h,self.current[1]-1,self.current[2],self.current)
    ...
    def printmaze(self):
        ...
        for element in path:
            ...
            self.path_cost+=1
        #write to file
        file = open("testfile_greedy.txt","w")
